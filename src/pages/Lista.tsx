...
const MBWAY_CONTACTS = copy.lista.paymentMethods.contacts
const REVOLUT_URL = 'https://revolut.me/joaommagalhaes'
const IBAN_VALUE = 'PT50 0023 0000 45479638251 94'
const IBAN_PREVIEW = 'PT50 00...'
...
function PaymentMethods({ amount, inline = false }: { amount: number; inline?: boolean }) {
    ...
    <div className="grid grid-cols-3 gap-2 sm:gap-3">
      <div className="flex h-full flex-col rounded-2xl border border-accent-mid/20 bg-accent-light/15 p-2.5 sm:p-4">
        ...
        <div className="mt-2 space-y-1 sm:mt-4 sm:space-y-2.5">
          {MBWAY_CONTACTS.map((contact) => (
            <div key={contact.number} className="px-0 py-0.5 text-center sm:text-left">
              <div className="min-w-0">
                <button
                  type="button"
                  onClick={() => copyValue(contact.number, contact.number)}
              ...
              {copied === contact.number && <p className="mt-1 text-[11px] font-medium text-accent sm:mt-2 sm:text-xs">{copy.lista.paymentMethods.copied.number}</p>}
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="flex h-full flex-col rounded-2xl border border-accent-mid/20 bg-accent-light/15 p-2.5 sm:p-4">
        ...
        <div className="mt-[0.55rem] text-center sm:mt-4 sm:pt-1.5 sm:text-left">
          ...
          <button
            type="button"
            onClick={() => copyValue('RevolutTag', 'joaommagalhaes')}
      ...
      <div className="flex h-full flex-col rounded-2xl border border-accent-mid/20 bg-accent-light/15 p-2.5 sm:p-4">
        ...
        <div className="mt-[0.6rem] text-center sm:mt-4 sm:pt-0.5 sm:text-left">
          <button
            type="button"
            onClick={() => copyValue('IBAN', IBAN_VALUE)}
          ...
          >
            <span className="break-all">{IBAN_VALUE}</span>
  ...
  return (
    <>
      ...
      >
        {copy.lista.paymentMethods.openLabel}
      </button>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed inset-0 z-50 flex items-center justify-center bg-forest/40 p-6 backdrop-blur-sm"
          ...
          >
            <motion.div
              ...
              className="w-full max-w-xl rounded-[28px] border border-accent-mid/20 bg-white p-5 shadow-2xl shadow-forest/20 sm:p-6"
    ...
    >
      ...
      <div className="aspect-[4/3] overflow-hidden bg-accent-light/50 relative">
        ...
        {isFull && (
          <div className="absolute inset-0 bg-white/70 backdrop-blur-sm flex items-center justify-center">
            ...
            <span className="text-xs font-medium text-accent-dark">{copy.lista.giftCard.full}</span>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="p-5 flex flex-col gap-3 flex-1">
        ...
        {!isFull && (
          ...
          >
            {copy.lista.giftCard.contribute}
          </button>
        )}
      </div>
    </motion.div>
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="bg-white rounded-t-3xl md:rounded-3xl p-5 md:p-7 max-w-lg w-full shadow-2xl shadow-forest/20 border border-accent-mid/20 max-h-[92vh] overflow-y-auto"
        onClick={(e) => e.stopPropagation()}
      >
        {done ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="grid gap-4 sm:grid-cols-[0.95fr_1.05fr]">
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder={copy.lista.contributionModal.namePlaceholder}
                    required
              ...
              <div className="flex flex-col-reverse gap-3 sm:flex-row sm:items-center">
                <button type="button" onClick={onClose} className="h-12 rounded-2xl px-5 text-sm font-medium text-accent-dark bg-accent-light hover:bg-accent-mid/30 transition-colors sm:flex-1">
                  {copy.lista.contributionModal.cancel}
                </button>
                <button
                  type="submit"
...
function HoneymoonFund() {  const [custom, setCustom] = useState('')
  ...
  return (
    <FadeUp>
      <div className="overflow-hidden rounded-[32px] border border-accent-mid/20 bg-[linear-gradient(180deg,_#f9fcfb_0%,_#edf6f4_100%)] shadow-[0_30px_80px_-40px_rgba(12,61,53,0.25)]">
        <div className="grid md:grid-cols-[0.95fr_1.05fr]">
          <div className="relative px-6 pb-8 pt-12 sm:px-8 md:px-10 md:py-12">
            <div className="absolute top-0 right-0 h-72 w-72 pointer-events-none"
              style={{ background: 'radial-gradient(circle, rgba(107,181,173,0.16) 0%, transparent 68%)' }} />
            <div className="absolute bottom-0 left-0 h-40 w-40 pointer-events-none"
              style={{ background: 'radial-gradient(circle, rgba(58,158,143,0.08) 0%, transparent 70%)' }} />

            <div className="relative">
              <p className="mb-4 text-xs font-medium uppercase tracking-widest text-accent">{copy.lista.honeymoon.tag}</p>
              <h2 className="mb-4 font-serif text-3xl text-forest sm:text-4xl md:text-5xl">{copy.lista.honeymoon.title}</h2>
              {done ? (
                <motion.div
                  {...motionProps({
                ...
                >
                  <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-accent/15">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#3A9E8F" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                      <polyline points="20 6 9 17 4 12" />
                    </svg>
                  </div>
                  <div>
                    <p className="font-medium text-forest">{copy.lista.honeymoon.successTitle}</p>
                    <p className="text-sm text-gray-500">{copy.lista.honeymoon.successMessage(finalAmount)}</p>
                  </div>
                </motion.div>
              ) : (
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div>
                    <label className="mb-3 block text-xs uppercase tracking-wider text-gray-500">{copy.lista.honeymoon.amountLabel}</label>
                    <div className="relative">
                      ...
                      <input
                        ...
                        placeholder={copy.lista.honeymoon.amountPlaceholder}
                  ...
                  <div>
                    <label className="mb-3 block text-xs uppercase tracking-wider text-gray-500">{copy.lista.honeymoon.nameLabel}</label>
                    <input
                      type="text"
                      value={name}
                      onChange={(e) => setName(e.target.value)}
                      placeholder={copy.lista.honeymoon.namePlaceholder}
                      required
                  ...
                  <div>
                    <label className="mb-3 block text-xs uppercase tracking-wider text-gray-500">{copy.lista.honeymoon.messageLabel}</label>
                    <textarea
                      value={message}
                      onChange={(e) => setMessage(e.target.value)}
                      placeholder={copy.lista.honeymoon.messagePlaceholder}
                      rows={3}
                  ...
                  <div className="flex w-full flex-col items-stretch gap-4">
                    ...
                    >
                      {loading ? copy.lista.honeymoon.loading : `${copy.lista.honeymoon.submit} ${finalAmount > 0 ? `${finalAmount}€` : ''}`}
                    </button>
                  </div>
          ...
          <div className="relative min-h-[22rem] border-t border-accent-mid/20 md:min-h-full md:border-l md:border-t-0">
            <img src={honeymoonImage} alt={copy.lista.honeymoon.imageAlt} className="h-full w-full object-cover" />
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <section className="pt-28 md:pt-32 pb-16 md:pb-20 px-4 sm:px-6 md:px-8 max-w-6xl mx-auto">
        ...
        >
          <p className="text-xs uppercase tracking-widest text-accent font-medium mb-5">{copy.lista.hero.subtitle}</p>
          <h1 className="font-serif text-4xl sm:text-5xl md:text-7xl text-forest mb-6 leading-tight">
            {copy.lista.hero.title}
          </h1>
        ...
        {loading ? (
          ...
          {gifts.length > collapsedGiftCount && (
            <FadeUp className="mt-8 flex justify-center">
              ...
              >
                {showAllGifts ? copy.lista.toggle.showLess : copy.lista.toggle.showMore}
              </button>
            </FadeUp>
          )}
          </>
      ...
      <footer className="py-10 bg-forest border-t border-white/5">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 md:px-8 flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-0 text-center sm:text-left">
          ...
          <p className="text-xs uppercase tracking-widest text-sage/60">{copy.lista.footer.date}</p>
        </div>
      </footer>
    </div>
  )
